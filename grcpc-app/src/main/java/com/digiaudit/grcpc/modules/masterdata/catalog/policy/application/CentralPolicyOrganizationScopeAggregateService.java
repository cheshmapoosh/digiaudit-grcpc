...
@Service
public class CentralPolicyOrganizationScopeAggregateService {
  ...
  public CentralPolicyOrganizationScopeAggregateService(CentralPolicyOrganizationScopeRepository relations,
      OrganizationRepository endpoints,
  ...
  public Prepared prepare(RevisionExecutionContext context, UUID policyId,
      ...
      List<CentralPolicyDtos.PolicyOrganizationScopeChange> requested) {
    guard.requireHierarchyGuard(context, MasterDataHierarchyKey.POLICY);
    List<CentralPolicyDtos.PolicyOrganizationScopeChange> changes = requested == null ? List.of() : new ArrayList<>(requested);
    if (!changes.isEmpty()) {
      guard.requireHierarchyGuard(context, MasterDataHierarchyKey.ORGANIZATION);
    }
    if (!changes.isEmpty()) { if (!authorization.canManage("GOVERNANCE") || !authorization.canView("REFERENCE"))
      throw new ForbiddenException("MASTER_DATA_ACCESS_DENIED", "error.security.forbidden", "Policy relation access denied"); }
    Set<UUID> seen = new HashSet<>();
    for (var change : changes) {
      if (change == null || change.operation() == null || change.organizationId() == null
          || !seen.add(change.organizationId()))
        throw invalid("Invalid or repeated Policy Organization relation change");
    }
    changes.sort(Comparator.comparing(CentralPolicyDtos.PolicyOrganizationScopeChange::organizationId));
    Map<UUID, Endpoint> lockedEndpoints = new HashMap<>();
    for (var change : changes) lockedEndpoints.put(change.organizationId(), lockEndpoint(change.organizationId()));
    Map<UUID, CentralPolicyOrganizationScopeEntity> existing = new HashMap<>();
    for (var row : relations.lockAllByPolicyId(policyId)) existing.put(row.getOrganizationId(), row);
    List<Mutation> prepared = new ArrayList<>();
    for (var change : changes) {
      CentralPolicyOrganizationScopeEntity row = existing.get(change.organizationId());
      ...
      if (operation != CentralPolicyDtos.PolicyScopeChangeOperation.DELETE) {
        if (!subset(from, to, ownerFrom, ownerTo)
            || !subset(from, to, lockedEndpoints.get(change.organizationId()).from(),
                lockedEndpoints.get(change.organizationId()).to()))
          throw invalid("Policy relation validity exceeds an endpoint");
      }
      RevisionOperationType revisionOp;
      if (operation == CentralPolicyDtos.PolicyScopeChangeOperation.CREATE_OR_RESTORE) {
        requireActive(ownerStatus, lockedEndpoints.get(change.organizationId()));
        if (row == null) {
          row = CentralPolicyOrganizationScopeEntity.create(policyId, change.organizationId(), from, to, actors.currentActorId(), Instant.now(clock));
        ...
        if (operation == CentralPolicyDtos.PolicyScopeChangeOperation.UPDATE) {
          ...
          if (change.requestedStatus() == MasterDataLifecycleStatus.ACTIVE) requireActive(ownerStatus, lockedEndpoints.get(change.organizationId()));
        } else if (operation == CentralPolicyDtos.PolicyScopeChangeOperation.ACTIVATE
            || operation == CentralPolicyDtos.PolicyScopeChangeOperation.RESTORE) requireActive(ownerStatus, lockedEndpoints.get(change.organizationId()));
    ...
    for (var row : existing.values()) {
      if (row.getStatus() == MasterDataLifecycleStatus.DELETED || seen.contains(row.getOrganizationId())) continue;
  ...
  public List<RevisionContentResult> apply(Prepared prepared, UUID policyId) {
    ...
    for (var mutation : prepared.mutations()) {
      ...
      contents.add(RevisionContentResult.completed(RevisionEntityType.CENTRAL_POLICY_ORGANIZATION_SCOPE,
  ...
  private JsonNode snapshot(CentralPolicyOrganizationScopeEntity row) {
    Map<String, Object> values = new LinkedHashMap<>();
    values.put("id", row.getId()); values.put("policyId", row.getPolicyId());
    values.put("organizationId", row.getOrganizationId());
  ...
  private record Mutation(CentralPolicyOrganizationScopeEntity row, RevisionOperationType operation, Long expected,
