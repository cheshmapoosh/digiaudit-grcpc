...
public class CentralRiskCategoryController {
    ...
    public CentralRiskCategoryController(CentralRiskCategoryCommandService commands, CentralRiskCategoryQueryService queries) {
        this.commands = commands;
        this.queries = queries;
    }

    @GetMapping @PreAuthorize("hasAuthority('CENTRAL_RISK_VIEW') or hasAuthority('ROLE_ROOT_ADMIN')")
    public List<CentralRiskCategorySummaryResponse> list() { return queries.list(); }
    @GetMapping("/deleted") @PreAuthorize("hasAuthority('CENTRAL_RISK_VIEW') or hasAuthority('ROLE_ROOT_ADMIN')")
    public List<CentralRiskCategorySummaryResponse> deleted() { return queries.listDeleted(); }
    @GetMapping("/tree") @PreAuthorize("hasAuthority('CENTRAL_RISK_VIEW') or hasAuthority('ROLE_ROOT_ADMIN')")
    public List<CentralRiskCategoryTreeResponse> tree() { return queries.tree(); }
    @GetMapping("/{id}") @PreAuthorize("hasAuthority('CENTRAL_RISK_VIEW') or hasAuthority('ROLE_ROOT_ADMIN')")
    public CentralRiskCategoryResponse detail(@PathVariable UUID id) { return queries.detail(id); }
    @PostMapping @PreAuthorize("hasAuthority('CENTRAL_RISK_CREATE') or hasAuthority('ROLE_ROOT_ADMIN')")
    public MasterDataAggregateMutationResponse create(@Valid @RequestBody CreateCentralRiskCategoryRequest request) { return commands.create(request); }
    @PatchMapping("/{id}") @PreAuthorize("hasAuthority('CENTRAL_RISK_UPDATE') or hasAuthority('ROLE_ROOT_ADMIN')")
    public MasterDataAggregateMutationResponse update(@PathVariable UUID id, @Valid @RequestBody UpdateCentralRiskCategoryRequest request) { return commands.update(id, request); }
    @PostMapping("/{id}/move") @PreAuthorize("hasAuthority('CENTRAL_RISK_MOVE') or hasAuthority('ROLE_ROOT_ADMIN')")
    public MasterDataRevisionMutationResponse move(@PathVariable UUID id, @Valid @RequestBody MoveCentralRiskCategoryRequest request) { return commands.move(id, request); }
