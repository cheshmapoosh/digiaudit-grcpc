package com.digiaudit.grcpc.modules.masterdata.classification.controlobjectiveaccountgroup.domain.entity;
...
@Entity
@Table(name = "central_control_objective_account_group")
public class CentralControlObjectiveAccountGroupEntity {
  @Id
  ...
  protected CentralControlObjectiveAccountGroupEntity() {}

  public static CentralControlObjectiveAccountGroupEntity create(
      UUID id, UUID controlObjectiveId, UUID accountGroupId,
      LocalDate validFrom, LocalDate validTo, UUID actorId, Instant now) {
    CentralControlObjectiveAccountGroupEntity value = new CentralControlObjectiveAccountGroupEntity();
    value.id = Objects.requireNonNull(id);
    value.controlObjectiveId = Objects.requireNonNull(controlObjectiveId);
  ...
  private void requireNotDeleted() {
    if (status == MasterDataLifecycleStatus.DELETED) throw new IllegalStateException("Deleted classification cannot be updated");
  }

  private void requireStatus(MasterDataLifecycleStatus expected) {
    if (status != expected) throw new IllegalStateException("Invalid classification lifecycle transition");
  }
