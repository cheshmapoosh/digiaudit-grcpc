...
public class CentralRiskTemplateCommandService {
    ...
    public CentralRiskTemplateCommandService(
            CentralRiskTemplateRepository repository, CentralRiskCategoryRepository categoryRepository,
    ...
    public MasterDataAggregateMutationResponse create(CreateCentralRiskTemplateRequest request) {
        ...
        try {
            RevisionExecutionResult result = revisions.executeStructural(MasterDataHierarchyKey.RISK,
                    RevisionRequest.central("Create risk template " + code, "Risk Template structural create", null),
                    context -> {
                        ...
                        if (entity == null) {
                            entity = CentralRiskTemplateEntity.create(UUID.randomUUID(), code, title, request.riskCategoryId(),
                                    description, sortOrder, request.validFrom(), request.validTo(), actor, now);
                            ...
                            if (!entity.getRiskCategoryId().equals(request.riskCategoryId())) throw parentMismatch();
                            expectedVersion = entity.getVersion();
                            before = snapshot(entity);
                            if (entity.getStatus() == MasterDataLifecycleStatus.DELETED) {
                                entity.restoreFromCreate(title, request.riskCategoryId(), description, sortOrder,
                                        request.validFrom(), request.validTo(), actor, now);
                                operationType = RevisionOperationType.RESTORE;
                            } else {
                                entity.reactivateFromCreate(title, request.riskCategoryId(), description, sortOrder,
                                        request.validFrom(), request.validTo(), actor, now);
                                operationType = RevisionOperationType.ACTIVATE;
                            }
                        }
                        CentralRiskTemplateEntity saved = repository.saveAndFlush(entity);
                        finalized.set(documents.finalizePreparedAggregate(prepared, DocumentLinkTargetType.CENTRAL_RISK_TEMPLATE, saved.getId(), "CENTRAL_RISK_CREATE"));
    ...
    public MasterDataAggregateMutationResponse update(UUID id, UpdateCentralRiskTemplateRequest request) {
        ...
        RevisionExecutionResult result = revisions.execute(
                RevisionRequest.central("Update risk template " + id, "Risk Template definition update", null),
                context -> {
                    var prepared = documents.prepareAggregate(request.documents());
                    ...
                    CentralRiskTemplateEntity saved = repository.saveAndFlush(entity);
                    finalized.set(documents.finalizePreparedAggregate(prepared, DocumentLinkTargetType.CENTRAL_RISK_TEMPLATE, id, "CENTRAL_RISK_UPDATE"));
    ...
    public MasterDataRevisionMutationResponse move(UUID id, MoveCentralRiskTemplateRequest request) {
        ...
        int sortOrder = support.normalizeSortOrder(request.sortOrder());
        RevisionExecutionResult result = revisions.executeStructural(MasterDataHierarchyKey.RISK,
                RevisionRequest.central("Move risk template " + id, "Risk Template category move", null),
                context -> {
                    requireGuard(context);
                    Map<UUID, CentralRiskCategoryEntity> categories = categories();
                    requireCategory(request.riskCategoryId(), categories);
                    ...
                    if (entity.getRiskCategoryId().equals(request.riskCategoryId()) && entity.getSortOrder() == sortOrder) throw noChange();
                    JsonNode before = snapshot(entity);
                    entity.move(request.riskCategoryId(), sortOrder, actors.currentActorId(), Instant.now(clock));
    ...
    private MasterDataRevisionMutationResponse lifecycle(UUID id, Long version, RevisionOperationType operationType) {
        ...
        RevisionExecutionResult result = revisions.executeStructural(MasterDataHierarchyKey.RISK,
                RevisionRequest.central(operationType + " risk template " + id, "Risk Template lifecycle", null),
                context -> {
                    requireGuard(context);
                    Map<UUID, CentralRiskCategoryEntity> categories = categories();
                    CentralRiskTemplateEntity entity = lock(id);
    ...
    private Map<UUID, CentralRiskCategoryEntity> categories() { return categoryRepository.findAllByOrderByIdAsc().stream().collect(Collectors.toMap(CentralRiskCategoryEntity::getId, Function.identity())); }
    private void requireCategory(UUID id, Map<UUID, CentralRiskCategoryEntity> categories) { hierarchySupport.requireParent(UUID.randomUUID(), id, categories, "Risk Category"); }
    ...
    private CentralRiskTemplateEntity lock(UUID id) { return repository.lockById(id).orElseThrow(() -> notFound(id)); }
    private NotFoundException notFound(UUID id) { return new NotFoundException("MASTER_DATA_NOT_FOUND", "error.masterdata.v2.notFound", "Risk Template not found", id); }
    private UnprocessableEntityException parentMismatch() { return new UnprocessableEntityException("INVALID_PARENT", "error.masterdata.v2.invalidParent", "Create cannot change the stored category"); }
    ...
    private Map<String, ?> typed(CentralRiskTemplateEntity e) { return Map.of("riskCategoryId", e.getRiskCategoryId(), "sortOrder", e.getSortOrder()); }
    private JsonNode snapshot(CentralRiskTemplateEntity e) { return support.snapshot(e, typed(e)); }
    private RevisionOperationResult completed(RevisionExecutionContext c, CentralRiskTemplateEntity e, RevisionOperationType o, Long v, JsonNode b) { return support.completed(c, e, RevisionEntityType.CENTRAL_RISK_TEMPLATE, o, v, b, typed(e)); }
    private boolean same(CentralRiskTemplateEntity e, String t, String d, UpdateCentralRiskTemplateRequest r) { return Objects.equals(e.getTitle(), t) && Objects.equals(e.getDescription(), d) && Objects.equals(e.getValidFrom(), r.validFrom()) && Objects.equals(e.getValidTo(), r.validTo()); }
