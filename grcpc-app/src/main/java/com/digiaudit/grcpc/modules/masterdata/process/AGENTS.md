...
- Prompt 7.6 extends the read-only Subprocess Account Groups projection to the exact-node union of Control Scope classifications and Control Objective Scope classifications. It deduplicates Account Group IDs, retains separate source contributions, filters unauthorized source paths, and adds no Subprocess command, draft, or persistence table.
- Prompt 7 relationship completion makes Subprocess `PATCH` the sole owner of four typed Central Coverage change sets. Coverage uses the existing `PROCESS` Guard and one revision transaction, locks the full endpoint union in Control/Risk/Control Objective/Requirement order, prepares all families before mutation, applies validated Coverage deletes before Scope deletes, and appends revision contents in the approved fixed family order. Subprocess Create has no Coverage fields.
