...
@Service
public class CentralPolicyControlScopeAggregateService {
  ...
  public CentralPolicyControlScopeAggregateService(CentralPolicyControlScopeRepository relations,
      CentralSubprocessControlScopeRepository endpoints, CentralSubprocessRepository subprocesses, CentralControlRepository controls,
    ...
    this.subprocesses = subprocesses; this.controls = controls;
  ...
  public Prepared prepare(RevisionExecutionContext context, UUID policyId,
      ...
      List<CentralPolicyDtos.PolicyControlScopeChange> requested) {
    guard.requireHierarchyGuard(context, MasterDataHierarchyKey.POLICY);
    List<CentralPolicyDtos.PolicyControlScopeChange> changes = requested == null ? List.of() : new ArrayList<>(requested);
    if (!changes.isEmpty()) {
    ...
    if (!changes.isEmpty()) { if (!authorization.canManage("GOVERNANCE") || !authorization.canView("PROCESS") || !authorization.canView("CONTROL"))
      throw new ForbiddenException("MASTER_DATA_ACCESS_DENIED", "error.security.forbidden", "Policy relation access denied"); }
    Set<UUID> seen = new HashSet<>();
    for (var change : changes) {
      if (change == null || change.operation() == null || change.centralControlScopeId() == null
          || !seen.add(change.centralControlScopeId()))
        throw invalid("Invalid or repeated Policy Control relation change");
    }
    changes.sort(Comparator.comparing(CentralPolicyDtos.PolicyControlScopeChange::centralControlScopeId));
    Map<UUID, Endpoint> lockedEndpoints = new HashMap<>();
    for (var change : changes) lockedEndpoints.put(change.centralControlScopeId(), lockEndpoint(change.centralControlScopeId()));
    Map<UUID, CentralPolicyControlScopeEntity> existing = new HashMap<>();
    for (var row : relations.lockAllByPolicyId(policyId)) existing.put(row.getCentralControlScopeId(), row);
    List<Mutation> prepared = new ArrayList<>();
    for (var change : changes) {
      CentralPolicyControlScopeEntity row = existing.get(change.centralControlScopeId());
      ...
      if (operation != CentralPolicyDtos.PolicyScopeChangeOperation.DELETE) {
        if (!subset(from, to, ownerFrom, ownerTo)
            || !subset(from, to, lockedEndpoints.get(change.centralControlScopeId()).from(),
                lockedEndpoints.get(change.centralControlScopeId()).to()))
          throw invalid("Policy relation validity exceeds an endpoint");
      }
      RevisionOperationType revisionOp;
      if (operation == CentralPolicyDtos.PolicyScopeChangeOperation.CREATE_OR_RESTORE) {
        requireActive(ownerStatus, lockedEndpoints.get(change.centralControlScopeId()));
        if (row == null) {
          row = CentralPolicyControlScopeEntity.create(policyId, change.centralControlScopeId(), from, to, actors.currentActorId(), Instant.now(clock));
        ...
        if (operation == CentralPolicyDtos.PolicyScopeChangeOperation.UPDATE) {
          ...
          if (change.requestedStatus() == MasterDataLifecycleStatus.ACTIVE) requireActive(ownerStatus, lockedEndpoints.get(change.centralControlScopeId()));
        } else if (operation == CentralPolicyDtos.PolicyScopeChangeOperation.ACTIVATE
            || operation == CentralPolicyDtos.PolicyScopeChangeOperation.RESTORE) requireActive(ownerStatus, lockedEndpoints.get(change.centralControlScopeId()));
    ...
    for (var row : existing.values()) {
      if (row.getStatus() == MasterDataLifecycleStatus.DELETED || seen.contains(row.getCentralControlScopeId())) continue;
  ...
  public List<RevisionContentResult> apply(Prepared prepared, UUID policyId) {
    ...
    for (var mutation : prepared.mutations()) {
      ...
      contents.add(RevisionContentResult.completed(RevisionEntityType.CENTRAL_POLICY_CONTROL_SCOPE,
  ...
  private Endpoint lockEndpoint(UUID endpointId) {
    ...
    var control = controls.lockById(scope.getControlId()).orElseThrow(() -> missing(endpointId));
    boolean eligible = scope.getStatus() == MasterDataLifecycleStatus.ACTIVE
        ...
        && subset(scope.getValidFrom(), scope.getValidTo(), control.getValidFrom(), control.getValidTo());
  ...
  private JsonNode snapshot(CentralPolicyControlScopeEntity row) {
    Map<String, Object> values = new LinkedHashMap<>();
    values.put("id", row.getId()); values.put("policyId", row.getPolicyId());
    values.put("centralControlScopeId", row.getCentralControlScopeId());
  ...
  private record Mutation(CentralPolicyControlScopeEntity row, RevisionOperationType operation, Long expected,
