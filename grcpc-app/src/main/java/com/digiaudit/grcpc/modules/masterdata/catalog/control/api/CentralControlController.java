...
public class CentralControlController {
    ...
    public CentralControlController(CentralControlCommandService commands, CentralControlQueryService queries) {
    ...
    public List<CentralControlSummaryResponse> list() { return queries.list(); }

    @GetMapping("/deleted")
    @PreAuthorize("hasAuthority('CENTRAL_CONTROL_VIEW') or hasAuthority('ROLE_ROOT_ADMIN')")
    public List<CentralControlSummaryResponse> deleted() { return queries.listDeleted(); }

    @GetMapping("/{id}")
    @PreAuthorize("hasAuthority('CENTRAL_CONTROL_VIEW') or hasAuthority('ROLE_ROOT_ADMIN')")
    public CentralControlResponse detail(@PathVariable UUID id) { return queries.detail(id); }

    @PostMapping
    @PreAuthorize("hasAuthority('CENTRAL_CONTROL_CREATE') or hasAuthority('ROLE_ROOT_ADMIN')")
    public MasterDataAggregateMutationResponse create(@Valid @RequestBody CreateCentralControlRequest request) {
        return commands.create(request);
    }

    @PatchMapping("/{id}")
    @PreAuthorize("hasAuthority('CENTRAL_CONTROL_UPDATE') or hasAuthority('ROLE_ROOT_ADMIN')")
    public MasterDataAggregateMutationResponse update(@PathVariable UUID id, @Valid @RequestBody UpdateCentralControlRequest request) {
        return commands.update(id, request);
    }

    @PostMapping("/{id}/activate")
    @PreAuthorize("hasAuthority('CENTRAL_CONTROL_LIFECYCLE') or hasAuthority('ROLE_ROOT_ADMIN')")
    public MasterDataRevisionMutationResponse activate(@PathVariable UUID id, @Valid @RequestBody CatalogLifecycleCommandRequest request) {
        return commands.activate(id, request.version());
    }

    @PostMapping("/{id}/inactivate")
    @PreAuthorize("hasAuthority('CENTRAL_CONTROL_LIFECYCLE') or hasAuthority('ROLE_ROOT_ADMIN')")
    public MasterDataRevisionMutationResponse inactivate(@PathVariable UUID id, @Valid @RequestBody CatalogLifecycleCommandRequest request) {
    ...
    public MasterDataRevisionMutationResponse delete(@PathVariable UUID id, @Valid @RequestBody CatalogLifecycleCommandRequest request) {
    ...
    public MasterDataRevisionMutationResponse restore(@PathVariable UUID id, @Valid @RequestBody CatalogLifecycleCommandRequest request) {
