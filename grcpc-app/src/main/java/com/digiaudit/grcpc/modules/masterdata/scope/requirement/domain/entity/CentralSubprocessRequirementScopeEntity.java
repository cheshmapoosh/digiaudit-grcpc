package com.digiaudit.grcpc.modules.masterdata.scope.requirement.domain.entity;
...
@Entity
@Table(name = "central_subprocess_requirement_scope")
public class CentralSubprocessRequirementScopeEntity {
  @Id
  ...
  @JdbcTypeCode(SqlTypes.BINARY)
  @Column(name = "requirement_id", nullable = false, updatable = false, columnDefinition = "RAW(16)")
  ...
  protected CentralSubprocessRequirementScopeEntity() {}

  private CentralSubprocessRequirementScopeEntity(
      UUID id, UUID subprocessId, UUID requirementId, LocalDate validFrom,
    ...
    this.requirementId = Objects.requireNonNull(requirementId, "requirementId is required");
  ...
  public static CentralSubprocessRequirementScopeEntity create(
      UUID id, UUID subprocessId, UUID requirementId, LocalDate validFrom,
      LocalDate validTo, UUID actorId, Instant now) {
    return new CentralSubprocessRequirementScopeEntity(
        id, subprocessId, requirementId, validFrom, validTo, actorId, now);
  }
  ...
  public void delete(UUID actorId, Instant now) {
    if (status != MasterDataLifecycleStatus.ACTIVE && status != MasterDataLifecycleStatus.INACTIVE) {
      throw new IllegalStateException("Only a nondeleted Requirement Scope can be deleted");
    }
  ...
  private void requireNotDeleted() {
    if (status == MasterDataLifecycleStatus.DELETED) {
      throw new IllegalStateException("Deleted Requirement Scope cannot be updated");
    }
  }

  private void requireStatus(MasterDataLifecycleStatus required) {
    if (status != required) throw new IllegalStateException("Invalid Requirement Scope lifecycle transition");
  }
