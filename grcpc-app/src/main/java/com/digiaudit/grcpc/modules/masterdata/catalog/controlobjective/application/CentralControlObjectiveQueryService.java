...
public class CentralControlObjectiveQueryService {
    ...
    public CentralControlObjectiveQueryService(CentralControlObjectiveRepository repository, CentralControlObjectiveMapper mapper) {
    ...
    public List<CentralControlObjectiveSummaryResponse> list() {
        return repository.findByStatusNotOrderByTitleAscIdAsc(MasterDataLifecycleStatus.DELETED)
                .stream().map(mapper::toSummary).toList();
    }

    @Transactional(readOnly = true)
    public List<CentralControlObjectiveSummaryResponse> listDeleted() {
        return repository.findByStatusOrderByTitleAscIdAsc(MasterDataLifecycleStatus.DELETED)
                .stream().map(mapper::toSummary).toList();
    }
    ...
    public CentralControlObjectiveResponse detail(UUID id) {
        return repository.findByIdAndStatusNot(id, MasterDataLifecycleStatus.DELETED)
                ...
                .orElseThrow(() -> new NotFoundException("MASTER_DATA_NOT_FOUND", "error.masterdata.v2.notFound", "Control Objective not found", id));
    }
}
