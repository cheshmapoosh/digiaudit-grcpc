...
public class CatalogCommandSupport {
    ...
    public String normalizeCode(String code) {
        if (code == null || code.isBlank()) {
            throw new UnprocessableEntityException("DUPLICATE_BUSINESS_KEY", "error.masterdata.v2.codeRequired", "Code is required");
        }
    ...
    public String normalizeTitle(String title) {
        ...
        if (normalized.length() > 255) {
            throw new UnprocessableEntityException("INVALID_TITLE", "error.masterdata.v2.titleLength", "Title exceeds 255 characters");
        }
        return normalized;
    }
    ...
    public void validateValidity(LocalDate validFrom, LocalDate validTo) {
        if (validFrom != null && validTo != null && validTo.isBefore(validFrom)) {
            throw new UnprocessableEntityException("DATE_RANGE_INVALID", "error.masterdata.v2.invalidValidityRange", "Validity range is invalid");
        }
    }

    public long requireVersion(Long version) {
        if (version == null || version < 0) {
            throw new ConflictException("VERSION_CONFLICT", "error.masterdata.v2.versionConflict", "Expected version is required");
        }
        return version;
    }
    ...
    public void assertVersion(CentralDefinitionEntity entity, long expectedVersion) {
        if (entity.getVersion() != expectedVersion) {
            throw new ConflictException("VERSION_CONFLICT", "error.masterdata.v2.versionConflict", "The record has changed", entity.getId());
        }
    }

    public void validateLifecycle(CentralDefinitionEntity entity, RevisionOperationType operationType) {
        ...
        if (!valid) {
            throw new UnprocessableEntityException("INVALID_LIFECYCLE_TRANSITION", "error.masterdata.v2.invalidLifecycleTransition", "Invalid lifecycle transition");
        }
    }

    public ConflictException duplicate(String code) {
        return new ConflictException("DUPLICATE_BUSINESS_KEY", "error.masterdata.v2.duplicateBusinessKey", "A record with this code already exists", code);
    }
    ...
    public RevisionOperationResult completed(
        ...
        return RevisionOperationResult.completed(context, primary, List.of(RevisionContentResult.completed(
                entityType,
                entity.getId(),
                operationType,
                expectedVersion,
    ...
    public MasterDataAggregateMutationResponse aggregateResponse(
        ...
        return new MasterDataAggregateMutationResponse(primary.entityId(), primary.revisionId(), primary.version(), documents);
    }
}
