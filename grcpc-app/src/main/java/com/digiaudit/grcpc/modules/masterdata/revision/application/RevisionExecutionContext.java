...
public record RevisionExecutionContext(
    ...
    public static RevisionExecutionContext from(MasterDataRevision revision) {
        Objects.requireNonNull(revision, "revision is required");
        return new RevisionExecutionContext(revision.id(), revision.domain(), revision.organizationId(), revision.status());
    }
