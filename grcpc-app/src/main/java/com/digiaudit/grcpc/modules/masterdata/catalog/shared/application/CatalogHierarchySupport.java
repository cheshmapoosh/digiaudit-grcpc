...
public class CatalogHierarchySupport {
    public <T extends CentralDefinitionEntity> T requireParent(
        ...
        if (parentId.equals(targetId)) {
            throw new UnprocessableEntityException("HIERARCHY_SELF_PARENT", "error.masterdata.v2.hierarchySelfParent", "An item cannot be its own parent");
        }
        T parent = hierarchy.get(parentId);
        if (parent == null || parent.getStatus() == MasterDataLifecycleStatus.DELETED) {
            throw new UnprocessableEntityException("PARENT_NOT_FOUND", "error.masterdata.v2.parentNotFound", parentLabel + " not found", parentId);
        }
        return parent;
    }
    ...
    public <T extends CentralDefinitionEntity> void rejectCycle(
            ...
            UUID parentId,
            Map<UUID, T> hierarchy,
            Function<T, UUID> parentIdReader
    ) {
        Set<UUID> visited = new HashSet<>();
        UUID cursor = parentId;
        while (cursor != null) {
            if (cursor.equals(targetId) || !visited.add(cursor)) {
                throw new UnprocessableEntityException("HIERARCHY_CYCLE", "error.masterdata.v2.hierarchyCycle", "The move would create a hierarchy cycle");
            }
