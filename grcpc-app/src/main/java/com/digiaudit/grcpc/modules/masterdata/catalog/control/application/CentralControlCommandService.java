...
public class CentralControlCommandService {
    ...
    public MasterDataAggregateMutationResponse create(CreateCentralControlRequest request) {
        ...
        try {
            RevisionExecutionResult result = revisionCoordinator.execute(
                    RevisionRequest.central("Create central control " + code, "Central Control definition create", null),
                    context -> {
                        ...
                        if (entity == null) {
                            entity = CentralControlEntity.create(UUID.randomUUID(), code, title, description,
                        ...
                        documents.set(documentCommandService.finalizePreparedAggregate(
                                prepared, DocumentLinkTargetType.CENTRAL_CONTROL, saved.getId(), "CENTRAL_CONTROL_CREATE"));
    ...
    public MasterDataAggregateMutationResponse update(UUID id, UpdateCentralControlRequest request) {
        ...
        AtomicReference<List<DocumentCommandResponse>> documents = new AtomicReference<>(List.of());
        RevisionExecutionResult result = revisionCoordinator.execute(
                RevisionRequest.central("Update central control " + id, "Central Control definition update", null),
                context -> {
                    DocumentCommandService.PreparedAggregateContext prepared = documentCommandService.prepareAggregate(request.documents());
                    ...
                    CentralControlEntity saved = repository.saveAndFlush(entity);
                    documents.set(documentCommandService.finalizePreparedAggregate(
                            prepared, DocumentLinkTargetType.CENTRAL_CONTROL, saved.getId(), "CENTRAL_CONTROL_UPDATE"));
    ...
    private MasterDataRevisionMutationResponse lifecycle(UUID id, Long requestedVersion, RevisionOperationType operationType) {
        ...
        RevisionExecutionResult result = revisionCoordinator.execute(
                RevisionRequest.central(operationType + " central control " + id, "Central Control lifecycle command", null),
                context -> {
                    CentralControlEntity entity = lock(id);
                    support.assertVersion(entity, expectedVersion);
    ...
    private RevisionOperationResult completed(RevisionExecutionContext context, CentralControlEntity entity,
                                              RevisionOperationType operationType, Long expectedVersion, JsonNode before) {
        return support.completed(context, entity, RevisionEntityType.CENTRAL_CONTROL, operationType,
    ...
    private NotFoundException notFound(UUID id) {
        return new NotFoundException("MASTER_DATA_NOT_FOUND", "error.masterdata.v2.notFound", "Control not found", id);
    }

    private boolean sameDefinition(CentralControlEntity entity, String title, String description,
