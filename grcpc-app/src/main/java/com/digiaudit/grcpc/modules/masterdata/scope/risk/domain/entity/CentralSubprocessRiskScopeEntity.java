package com.digiaudit.grcpc.modules.masterdata.scope.risk.domain.entity;
...
@Entity
@Table(name = "central_subprocess_risk_scope")
public class CentralSubprocessRiskScopeEntity {
  @Id
  ...
  @JdbcTypeCode(SqlTypes.BINARY)
  @Column(name = "risk_template_id", nullable = false, updatable = false, columnDefinition = "RAW(16)")
  ...
  protected CentralSubprocessRiskScopeEntity() {}

  private CentralSubprocessRiskScopeEntity(
      UUID id,
      UUID subprocessId,
      UUID riskTemplateId,
      LocalDate validFrom,
    ...
    this.riskTemplateId = Objects.requireNonNull(riskTemplateId, "riskTemplateId is required");
  ...
  public static CentralSubprocessRiskScopeEntity create(
      UUID id,
      UUID subprocessId,
      UUID riskTemplateId,
      LocalDate validFrom,
      ...
      UUID actorId,
      Instant now) {
    return new CentralSubprocessRiskScopeEntity(
        id, subprocessId, riskTemplateId, validFrom, validTo, actorId, now);
  }
  ...
  public void delete(UUID actorId, Instant now) {
    if (status != MasterDataLifecycleStatus.ACTIVE
      ...
      throw new IllegalStateException("Only a nondeleted risk scope can be deleted");
    }
  ...
  private void requireNotDeleted() {
    if (status == MasterDataLifecycleStatus.DELETED) {
      throw new IllegalStateException("Deleted risk scope cannot be updated");
    }
  }

  private void requireStatus(MasterDataLifecycleStatus required) {
    if (status != required) {
      throw new IllegalStateException("Invalid risk scope lifecycle transition");
    }
  }
