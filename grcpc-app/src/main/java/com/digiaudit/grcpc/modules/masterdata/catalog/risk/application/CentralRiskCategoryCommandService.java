...
public class CentralRiskCategoryCommandService {
    ...
    public CentralRiskCategoryCommandService(
            CentralRiskCategoryRepository repository, CentralRiskTemplateRepository templateRepository,
            MasterDataRevisionCoordinator revisions, MasterDataRevisionActorProvider actors,
            RevisionMutationGuard mutationGuard, DocumentCommandService documents,
    ...
    public MasterDataAggregateMutationResponse create(CreateCentralRiskCategoryRequest request) {
        ...
        try {
            RevisionExecutionResult result = revisions.executeStructural(MasterDataHierarchyKey.RISK,
                    RevisionRequest.central("Create risk category " + code, "Risk Category structural create", null),
                    context -> {
                        ...
                        hierarchySupport.requireParent(id, request.parentCategoryId(), hierarchy, "Risk Category parent");
                        hierarchySupport.rejectCycle(id, request.parentCategoryId(), hierarchy, CentralRiskCategoryEntity::getParentCategoryId);
                        if (entity == null) {
                            entity = CentralRiskCategoryEntity.create(id, code, title, request.parentCategoryId(), description,
                                    sortOrder, request.validFrom(), request.validTo(), actor, now);
                            ...
                            if (!Objects.equals(entity.getParentCategoryId(), request.parentCategoryId())) throw parentMismatch();
                            expectedVersion = entity.getVersion();
                            before = snapshot(entity);
                            if (entity.getStatus() == MasterDataLifecycleStatus.DELETED) {
                                entity.restoreFromCreate(title, request.parentCategoryId(), description, sortOrder,
                                        request.validFrom(), request.validTo(), actor, now);
                                operationType = RevisionOperationType.RESTORE;
                            } else {
                                entity.reactivateFromCreate(title, request.parentCategoryId(), description, sortOrder,
                                        request.validFrom(), request.validTo(), actor, now);
                                operationType = RevisionOperationType.ACTIVATE;
                            }
                        }
                        CentralRiskCategoryEntity saved = repository.saveAndFlush(entity);
                        finalized.set(documents.finalizePreparedAggregate(prepared, DocumentLinkTargetType.CENTRAL_RISK_CATEGORY, saved.getId(), "CENTRAL_RISK_CREATE"));
    ...
    public MasterDataAggregateMutationResponse update(UUID id, UpdateCentralRiskCategoryRequest request) {
        long expectedVersion = support.requireVersion(request.version());
        String title = support.normalizeTitle(request.title());
        String description = support.normalizeDescription(request.description());
        support.validateValidity(request.validFrom(), request.validTo());
        AtomicReference<List<DocumentCommandResponse>> finalized = new AtomicReference<>(List.of());
        RevisionExecutionResult result = revisions.execute(
                RevisionRequest.central("Update risk category " + id, "Risk Category definition update", null),
                context -> {
                    var prepared = documents.prepareAggregate(request.documents());
                    ...
                    if (same(entity, title, description, request) && empty(request.documents())) throw noChange();
                    JsonNode before = snapshot(entity);
                    entity.update(title, description, request.validFrom(), request.validTo(), actors.currentActorId(), Instant.now(clock));
                    CentralRiskCategoryEntity saved = repository.saveAndFlush(entity);
                    finalized.set(documents.finalizePreparedAggregate(prepared, DocumentLinkTargetType.CENTRAL_RISK_CATEGORY, id, "CENTRAL_RISK_UPDATE"));
    ...
    public MasterDataRevisionMutationResponse move(UUID id, MoveCentralRiskCategoryRequest request) {
        ...
        int sortOrder = support.normalizeSortOrder(request.sortOrder());
        RevisionExecutionResult result = revisions.executeStructural(MasterDataHierarchyKey.RISK,
                RevisionRequest.central("Move risk category " + id, "Risk Category hierarchy move", null),
                context -> {
                    requireGuard(context);
                    Map<UUID, CentralRiskCategoryEntity> hierarchy = hierarchy();
                    ...
                    hierarchySupport.requireParent(id, request.parentCategoryId(), hierarchy, "Risk Category parent");
                    hierarchySupport.rejectCycle(id, request.parentCategoryId(), hierarchy, CentralRiskCategoryEntity::getParentCategoryId);
                    if (Objects.equals(entity.getParentCategoryId(), request.parentCategoryId()) && entity.getSortOrder() == sortOrder) throw noChange();
                    JsonNode before = snapshot(entity);
                    entity.move(request.parentCategoryId(), sortOrder, actors.currentActorId(), Instant.now(clock));
                    return completed(context, repository.saveAndFlush(entity), RevisionOperationType.UPDATE, expectedVersion, before);
                });
    ...
    private MasterDataRevisionMutationResponse lifecycle(UUID id, Long version, RevisionOperationType operationType) {
        long expectedVersion = support.requireVersion(version);
        RevisionExecutionResult result = revisions.executeStructural(MasterDataHierarchyKey.RISK,
                RevisionRequest.central(operationType + " risk category " + id, "Risk Category lifecycle", null),
                context -> {
                    requireGuard(context);
                    Map<UUID, CentralRiskCategoryEntity> hierarchy = hierarchy();
                    CentralRiskCategoryEntity entity = hierarchy.get(id);
                    ...
                    if (operationType == RevisionOperationType.DELETE &&
                            (repository.existsByParentCategoryIdAndStatusNot(id, MasterDataLifecycleStatus.DELETED)
                                    || templateRepository.existsByRiskCategoryIdAndStatusNot(id, MasterDataLifecycleStatus.DELETED))) {
                        throw new ConflictException("DEPENDENCY_EXISTS", "error.masterdata.v2.dependencyExists", "Risk Category has nondeleted children", id);
                    }
                    if (operationType == RevisionOperationType.ACTIVATE || operationType == RevisionOperationType.RESTORE) {
                        hierarchySupport.requireParent(id, entity.getParentCategoryId(), hierarchy, "Risk Category parent");
                        hierarchySupport.rejectCycle(id, entity.getParentCategoryId(), hierarchy, CentralRiskCategoryEntity::getParentCategoryId);
                    }
                    JsonNode before = snapshot(entity);
                    UUID actor = actors.currentActorId();
                    Instant now = Instant.now(clock);
    ...
    private Map<UUID, CentralRiskCategoryEntity> hierarchy() {
        return repository.findAllByOrderByIdAsc().stream().collect(Collectors.toMap(CentralRiskCategoryEntity::getId, Function.identity()));
    }
    private void requireGuard(RevisionExecutionContext context) { mutationGuard.requireHierarchyGuard(context, MasterDataHierarchyKey.RISK); }
    private CentralRiskCategoryEntity lock(UUID id) { return repository.lockById(id).orElseThrow(() -> notFound(id)); }
    private NotFoundException notFound(UUID id) { return new NotFoundException("MASTER_DATA_NOT_FOUND", "error.masterdata.v2.notFound", "Risk Category not found", id); }
    private UnprocessableEntityException parentMismatch() { return new UnprocessableEntityException("INVALID_PARENT", "error.masterdata.v2.invalidParent", "Create cannot change the stored parent"); }
    private UnprocessableEntityException noChange() { return new UnprocessableEntityException("NO_CHANGE", "error.masterdata.v2.noChange", "The command contains no change"); }
    private JsonNode snapshot(CentralRiskCategoryEntity e) { return support.snapshot(e, typed(e)); }
    private Map<String, ?> typed(CentralRiskCategoryEntity e) { return Map.of("parentCategoryId", e.getParentCategoryId() == null ? "" : e.getParentCategoryId(), "sortOrder", e.getSortOrder()); }
    private RevisionOperationResult completed(RevisionExecutionContext c, CentralRiskCategoryEntity e, RevisionOperationType o, Long v, JsonNode b) { return support.completed(c, e, RevisionEntityType.CENTRAL_RISK_CATEGORY, o, v, b, typed(e)); }
    private boolean same(CentralRiskCategoryEntity e, String t, String d, UpdateCentralRiskCategoryRequest r) { return Objects.equals(e.getTitle(), t) && Objects.equals(e.getDescription(), d) && Objects.equals(e.getValidFrom(), r.validFrom()) && Objects.equals(e.getValidTo(), r.validTo()); }
