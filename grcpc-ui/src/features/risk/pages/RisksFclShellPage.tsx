...
import type { RiskNode, RiskNodeCreate, RiskNodeType, RiskNodeUpdate } from "../domain/risk.model";
...
function mapError(
    ...
    if (error instanceof Error && error.message) {
        switch (error.message) {
            case "NOT_FOUND":
                return t("risk.errors.notFound", { defaultValue: "آیتم موردنظر یافت نشد" });
            case "PARENT_NOT_FOUND":
                return t("risk.errors.parentNotFound", { defaultValue: "والد انتخاب‌شده یافت نشد" });
            case "HAS_CHILDREN":
...
export default function RisksFclShellPage() {
    ...
    const handleCancel = useCallback(() => {
        const currentAnchorId =
            routeMode === "create" ? queryParentId ?? selectedTreeId : riskId ?? selectedTreeId;
    ...
    const listColumn = createElement(
        ...
        <div style={frameStyle}>
            <RisksListReport
                ...
                createOptions={createOptions.length ? createOptions : CREATE_NODE_TYPES}
