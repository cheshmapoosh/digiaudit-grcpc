...
Policy Version target wording in the original inventory is superseded by the approved [Policy simplification](policy-simplification-migration.md): active Policy owns content and scopes; historical `central_policy_version` rows remain stored but unmapped.
...
| `REPLACE` | `V1080` / `policy_node` | Combined Policy Group/Policy with mutable version/workflow behavior | Active model separates Group and Policy; Policy owns content and lifecycle, with no publication workflow. | `central_policy_group`, `central_policy`; `central_policy_version` retained solely as historical storage. | P4 |
...
| `REPLACE` | `modules/masterdata/policy/**` | Combined policy tree, mutable content/status/workflow-oriented APIs | Requires Group → Policy and no approval workflow persistence. | Policy-owned content and four typed Policy-scope collections. | P4–P6 |
...
| `KEEP_VISUAL_REPLACE_DATA_FLOW` | `features/policy/pages/PoliciesFclShellPage.tsx`, policy tree/state/API repo | Policy FCL/tree/version UI | Former model merged hierarchy/version and workflow-oriented actions. | Keep visual patterns; bind Policy Group, Policy content, and four Policy-owned scopes. | P4–P6 |
