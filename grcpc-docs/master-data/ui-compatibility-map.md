...
| Organization General Information | Organization General Information form | `organization` | `REMAP_COMPLETE` | Prompt 5.10 consumes `code`, `name`, `organizationType`, `parentOrganizationId`, `status`, `location`, `validFrom`, `validTo`, and `description`; `displayLabel` is `name`. Create remains server-owned `ACTIVE`; parent is editable on Create and Edit. |
| Organization aggregate modal save | Organization ObjectPage form | Organization aggregate Create/Update command | `REMAP_COMPLETE` | One Save contains details, typed parent, `ACTIVE`/`INACTIVE`, and declarative Document drafts. It does not chain Update, Move, or Document mutation calls; Backend Move/Delete/Restore commands may remain without target Move UI. |
...
| Parent/owner editing | Current tree behavior | Process parent and Subprocess owner | `KEEP_VISUAL_REPLACE_DATA_FLOW` | Use the typed selector in Create and Edit. Modal Save carries `parentProcessId` or required `processId`; no separate Move button/dialog or browser Move call remains. |
| Process/Subprocess General Information | Shared typed node form | Separate Process/Subprocess aggregate Update DTOs | `REMAP_COMPLETE` | The shared visual form sends details, `ACTIVE`/`INACTIVE`, typed parent/owner, and Document drafts in one request and never includes code or generic parent IDs. |
...
Prompt 5.10 closes the Prompt 5 compatibility gap. Organization and Process/Subprocess screens expose only General Information and Documents tabs; visible structural toolbar actions are Create, View, and Delete, and modal footers have no Move or Document Save action. One aggregate Save owns parent/owner, details, and Document drafts. Inactive nodes remain visible and selectable with semantic Critical presentation. All target business dates use the shared Persian UI5 DatePicker, retain invalid typed drafts, and submit Gregorian `yyyy-MM-dd`; free-text Document Category remains hidden.
...
| Controlled Document Link | Generic `targetType`/`targetId` binding | `document_link` | `REPLACE` | Standalone UI receives an approved typed target context. Prompt 5.10 parent-save drafts omit target type/ID because the owning endpoint derives them; no open generic target list or internal revision metadata target is exposed. |
